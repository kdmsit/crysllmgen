...
login(token = "<Add your Hugging Face Token>")
...
class CifDataset(Dataset):
    ...
    def generation_task(self, input_dict):
        ...
        if num_attributes > 0 and self.w_attributes:
            ...
            attributes = ["pretty_formula"] + attributes   #pretty_formula
...
def setup_datasets(args, llama_tokenizer,dataset, transform_args={}):
    format_options = {
...
def setup_model(args, rank):
    ...
    # model = LlamaForCausalLM.from_pretrained(model_string,load_in_8bit=args.fp8, device_map={"": rank})
    model = LlamaForCausalLM.from_pretrained(model_string)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--eval-freq", default=1000, type=int) #1000
    parser.add_argument("--save-freq", default=5000, type=int) #500
